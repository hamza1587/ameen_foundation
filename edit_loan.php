...
<?php $sub_page = "edit_loan"; ?>
<?php $title = "Edit Loan"; ?>
<?php include "includes/main_sidebar.php"; ?>
<?php
if ($isAdmin != 1) {
    if ($loans_access == FALSE) {
...
if ($stmt1->num_rows > 0) {
    ...
    if($payment_type == "Credit"){
        $stmt2 = $conn->prepare('SELECT loans.person_id, name, mobile_no, details, address, invoice_no, bank_acc_id, credit, date FROM loans INNER JOIN person_information ON loans.person_id = person_information.person_id WHERE loan_id = ?');
        $stmt2->bind_param('s', $loan);
        $stmt2->execute();
        $stmt2->store_result();
        $stmt2->bind_result($person_id, $name, $credit_mobile_no, $credit_details, $credit_address, $credit_invoice_no, $credit_bank_acc_id, $credit, $credit_date);
        $stmt2->fetch();
        $stmt2->close();
    }else{
        $stmt3 = $conn->prepare('SELECT loans.person_id, mobile_no, details, address, invoice_no, bank_acc_id, credit, debit, date FROM loans INNER JOIN person_information ON loans.person_id = person_information.person_id WHERE loan_id = ?');
        $stmt3->bind_param('s', $loan);
        $stmt3->execute();
        $stmt3->store_result();
        $stmt3->bind_result($person_id, $debit_mobile_no, $debit_details, $debit_address, $debit_invoice_no, $debit_bank_acc_id, $credit, $debit, $debit_date);
        $stmt3->fetch();
        $stmt3->close();
    }
}
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <?php include "loan_management/edit.php"; ?>
                                <form method="post">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-4">
                                                                <div class="form-group">
                                                                    ...
                                                                    <input type="number" name="credit_mobile_no" id="mobile_no" value="<?= $credit_mobile_no; ?>" class="form-control" required="" placeholder="Enter Mobile No">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <div class="form-group">
                                                                    <label for="firstname" class="control-label" required="">Bank Account</label>
                                                                    <select class="form-control" name="credit_bank_acc_id" id="bank_acc_id">
                                                    ...
                                                    <div class="card-body">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-4">
                                                                ...
                                                                <input type="date" name="credit_date" class="form-control" value="<?= $credit_date; ?>" required="" <?php if ($isAdmin != 1) {
                                                                    echo "readonly";
                                                                } ?>>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <label for="date" class="control-label">Amount</label>
                                                                <input type="number" name="credit" id="amount" class="form-control" value="<?= $credit; ?>" placeholder="Enter Amount Here" required="">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="card">
                                                    <div class="card-body">
                                                        <div class="form-group">
                                                            ...
                                                            <textarea name="credit_address" id="address" class="form-control" rows="2" placeholder=""><?= $credit_address ?></textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="card">
                                                    <div class="card-body">
                                                        <div class="form-group">
                                                            ...
                                                            <textarea name="credit_details" id="details" class="form-control" rows="2" placeholder=""><?= $credit_details ?></textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="card-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-4">
                                                            <div class="form-group">
                                                                <label for="firstname" class="control-label" required="">Bank Account</label>
                                                                <select class="form-control" name="debit_bank_acc_id" id="bank_acc_id">
                                                                    <option value="" selected>Choose Cash Account</option>
                                                                    <?php
                                                                    $fetch_bank_accounts = "select * from bank_accounts";
                                                                    $run_bank_accounts = mysqli_query($conn, $fetch_bank_accounts);
                                            ...
                                            <div class="card">
                                                <div class="card-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-3">
                                                            <label for="date" class="control-label">Billing Date</label>
                                                            <input type="date" name="debit_date" class="form-control" value="<?= $debit_date; ?>" required="" <?php if ($isAdmin != 1) {
                                                                echo "readonly";
                                                            } ?>>
                                                        </div>
                                                        <div class="col-md-3">
