...
if (!isset($_SESSION['loggedin'])) {
    ...
    $stmt = $conn->prepare('SELECT `receipt_no`, `donator_name`, `phone_number`, `total_amount_num`, `total_amount_words`, `amount_type`, `bank_acc_name`, `income_source_title`, `cheque_no`, `total`, `date`, `description`, `address` FROM donations INNER JOIN income_source ON donations.income_source_id = income_source.income_source_id INNER JOIN bank_accounts ON donations.bank_acc_id = bank_accounts.bank_acc_id WHERE donation_id = ?');
    $stmt->bind_param('s', $_GET['invoice']);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows > 0) {
        $stmt->bind_result($receipt_no, $donator_name, $phone_number, $total_amount_num, $total_amount_words, $amount_type ,$bank_acc_name, $income_source_title, $cheque_no, $total, $date, $description, $address);
        $stmt->fetch();
...
<body>
        ...
        <main>
            <div class="row">
                ...
                <div class="col-sm-6 text-sm-left order-sm-0"> <strong>Invoiced To:</strong>
                    <address>
                        <p class="text-sm-left">Name: <strong> <?= $donator_name; ?></strong></p>
                        <p class="text-sm-left">Phone No: <strong><?= $phone_number; ?></strong></p>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                <tr>
                                    <td><span class="text-3"><?= $donator_name; ?></span></td>
                                    <td class="text-center"><?= $bank_acc_name; ?></td>
                                    <td class="text-right"><?php echo number_format($total_amount_num, 2); ?></td>
                                </tr>
                                <tr>
                                    <td colspan="3">Donation For: <br>
                                ...
                                <tr>
                                    <td colspan="3">Donated Amount in Words: <br>
                                        <small><?= $total_amount_words; ?></small>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="2" class="bg-light-2 text-right"><strong>Total</strong></td>
                                    <td class="bg-light-2 text-right"><?php echo number_format($total_amount_num, 2); ?>
                                        Rs</td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <?php } ?>
                            <td class="text-center"><?php echo number_format($total_amount_num, 2); ?> Rs</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <p class="text-center d-print-none"><a href="../add-donation">&laquo; Go Back</a></p>
</body>
